...
class CircuitBreaker:
    ...
    def allow_request(self) -> bool:
        """Allow closed/half-open calls and release an open circuit after its timeout."""
        if self.state in {CircuitState.CLOSED, CircuitState.HALF_OPEN}:
            return True

        if self.opened_at is None:
            # A defensive fallback for state restored without its timestamp: start
            # the timeout now instead of immediately flooding the provider.
    ...
    def call(self, fn: Callable[..., T], *args: object, **kwargs: object) -> T:
        """Call a function through the breaker and record its outcome."""
    ...
    def record_failure(self) -> None:
        """Open on the closed-state threshold or immediately after a failed probe."""
