...
class ResponseCache:
    """TTL-aware in-memory semantic cache with privacy and false-hit guardrails."""
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        """Return the best unexpired safe semantic match and its similarity score."""
    ...
    def set(self, query: str, value: str, metadata: dict[str, str] | None = None) -> None:
        """Store a response unless its query is privacy-sensitive."""
        if _is_uncacheable(query):
            return
    ...
    def similarity(a: str, b: str) -> float:
        """Compute cosine similarity over lowercase words and character trigrams."""
        if a == b:
            return 1.0
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        """Return an exact or best safe semantic match from Redis."""
        if _is_uncacheable(query):
            return None, 0.0
    ...
    def set(self, query: str, value: str, metadata: dict[str, str] | None = None) -> None:
        """Store a non-sensitive response in a Redis hash with a TTL."""
        if _is_uncacheable(query):
            return
